...
void CPUTSlider::RecalculateSizes()
{
    ...
    {
        mpActiveImageSizeList[QuadIndex].width      = mScale * (gUVLocationsSlider_active[ii+2] - gUVLocationsSlider_active[ii+0]);
        mpActiveImageSizeList[QuadIndex].height     = mScale * (gUVLocationsSlider_active[ii+3] - gUVLocationsSlider_active[ii+1]);

        mpPressedImageSizeList[QuadIndex].width     = mScale * (gUVLocationsSlider_pressed[ii+2] - gUVLocationsSlider_pressed[ii+0]);
        mpPressedImageSizeList[QuadIndex].height    = mScale * (gUVLocationsSlider_pressed[ii+3] - gUVLocationsSlider_pressed[ii+1]);

        mpDisabledImageSizeList[QuadIndex].width    = mScale * (gUVLocationsSlider_disabled[ii+2] - gUVLocationsSlider_disabled[ii+0]);
        mpDisabledImageSizeList[QuadIndex].height   = mScale * (gUVLocationsSlider_disabled[ii+3] - gUVLocationsSlider_disabled[ii+1]);
        QuadIndex++;
    }
...

{
    LocationGuides guides;
    CalculateLocationGuides(guides);
    float UpperLeftX=mControlDimensions.x;
...
bool CPUTSlider::PointingAtNub(int x, int y)
{
    ...
    float LowerRightX = UpperLeftX + mpActiveImageSizeList[Grip].width*2;
    float LowerRightY = UpperLeftY + mpActiveImageSizeList[Grip].height*2;
#endif
    if( (x>LowerRightX) || (x<UpperLeftX) ||
        (y<UpperLeftY) || (y>LowerRightY ) )
